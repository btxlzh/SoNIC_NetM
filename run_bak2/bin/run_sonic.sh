...
function run_sonic {
    echo tester mode=$mode pkt_len=$pkt_len duration=$duration idle=$idle pkt_cnt=$pkt_cnt wait=$mac_wait mac_src=$mac_src mac_dst=$mac_dst ip_src=$ip_src ip_dst=$ip_dst port_src=$port_src port_dst=$port_dst vlan_id=$vlan_id gen_mode=$gen_mode delta=$epsilon chain_idle=$chain_idle chain_num=$chain_num> /proc/sonic_tester 
...
#    echo "Processing captured packets"
    processed=`$HOME/sonic/git/driver/bin/sonic_parse.py -i ${result_path} -d -s $mac_src -t $mac_dst | grep "Processed" | awk '{print $2}'`
}
